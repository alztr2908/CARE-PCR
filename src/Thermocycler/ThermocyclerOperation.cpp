...
float Vcc = 3.3; // Vcc of the port that supplies the thermistor divider
float Vout;      // Voltage across thermistor we measrure
float R_NTC;     // resistance value we calculate from Vout using voltage divider formula
float Temp_K;
float Temp_C;
float resistance_table[200] = {310.764,
                               303.08805,
                               295.4121,
                               288.16025,
                               280.9084,
...
float ReadTemp()
{
  ...
  Temp_C = ConvertToTemp((R_NTC / 1000.0));
...
float ConvertToTemp(float r_ntc)
{
  for (int i = 0; i < 200; i++)
  {
    if ((r_ntc < resistance_table[i]) && (r_ntc >= resistance_table[i + 1]))
    {
      return i / 2.0;
    }
  }
